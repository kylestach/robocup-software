...
void WindowEvaluator::obstacle_robot(vector<Window>& windows, Point origin,
    ...
    if (debug) {
        context.artist->drawLine(seg, QColor{"Red"}, "Debug");
    }
...
WindowingResult WindowEvaluator::eval_pt_to_seg(Point origin, Segment target) {
    ...
    if (debug) {
        context.artist->drawLine(target, QColor{"Blue"}, "Debug");
    }
    ...
    if (debug) {
        if (best) {
            context.artist->drawLine(Segment{origin, best->segment.center()},
                             QColor{"Green"}, "Debug");
        }
        for (Window& window : windows) {
            context.artist->drawLine(window.segment, QColor{"Green"}, "Debug");
            context.artist->drawText(std::to_string(window.shot_success),
