...
Processor::Processor(bool sim, bool defendPlus, VisionChannel visionChannel,
    ...
    _inputDeviceManager = std::make_unique<InputDeviceManager>(&_context);
...
void Processor::manualID(int value) {
    ...
    _inputDeviceManager->manualID(value);
}
...
bool Processor::joystickValid() const {
    return _inputDeviceManager->joystickValid();
}
...
void Processor::run() {
    ...
    while (_running) {
        ...
        for (OurRobot* robot : _context.state.self) {
            robot->setJoystickControlled(robot->shell() == _inputDeviceManager->manualID());
        }
...
void Processor::sendRadioData() {
    ...
    // Add RadioTx commands for visible/joystick robots
    std::vector<int> manualIds = getJoystickRobotIds();
    for (OurRobot* r : _context.state.self) {
        RobotIntent& intent = _context.robot_intents[r->shell()];
        intent.is_active = r->visible() || r->isJoystickControlled();
    }
}
