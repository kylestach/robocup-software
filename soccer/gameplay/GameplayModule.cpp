...
Geometry2d::ShapeSet Gameplay::GameplayModule::globalObstacles() const {
    ...
    if (!_context.artist->log_frame_->use_our_half()) {
        obstacles.add(_ourHalf);
    }

    if (!_context.artist->log_frame_->use_opponent_half()) {
...
void Gameplay::GameplayModule::run() {
    ...
    PyGILState_STATE state = PyGILState_Ensure();
    {
        try {
            ...
            getMainModule().attr("set_debug_artist")(&_context.artist);
        ...
        try {
            ...
            try {
                ...
                _context.artist->log_frame_->set_behavior_tree(bhvrTreeDesc);
            } catch (error_already_set) {
                PyErr_Print();
            }
        } catch (error_already_set) {
    ...
    if (_context.state->gameState.stayAwayFromBall() && _context.state->ball.valid) {
        _context.artist->drawCircle(_context.state->ball.pos,
