...
OurRobot::OurRobot(int shell, Context ctx)
    : Robot(shell, true), _context(ctx) {
    ...
    _motionControl = new MotionControl(this, ctx);
...
void OurRobot::addText(const std::string& text, const QColor& qc,
                       const std::string& layerPrefix) {
    Packet::DebugText* dbg = new Packet::DebugText;
    std::string layer = layerPrefix + std::to_string(shell());
    dbg->set_layer(_context.artist->findDebugLayer(layer));
    dbg->set_text(text);
...
Geometry2d::ShapeSet OurRobot::collectAllObstacles(
    ...
    if (_context.state->ball.valid) {
        // _context.artist->drawShape(ball_obs, Qt::gray,
